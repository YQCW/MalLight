...
malfunction = True     #QCY: I add this
...
class Intersection(object):
    ...
    def __init__(self, id, world, phases):
        ...
        self.disabled = False    #QCY: indicate whether this intersection is in malfunction
        self.program = None    #QCY: restore the program before intersection malfunction
        self.comingVehicles = set()    #QCY: I add this
        self.passedVehicles = set()   #QCY: I add this
        self.in_lanes = []    #QCY: I add this
        self.out_lanes = []    #QCY: I add this
        ...
        if self.disabled:    #QCY: I add this
            self.eng.trafficlight.setProgram(self.id, "off")
        ...
        for l in self.in_roads:    #QCY: I add this
            self.in_lanes.extend(self.road_lane_mapping[l])

        for l in self.out_roads:    #QCY: I add this
            self.out_lanes.extend(self.road_lane_mapping[l])
    ...
    def reset(self):
        ...
        logic.phases = self.full_phases
        self.eng.trafficlight.setProgramLogic(self.id, logic)
        if self.disabled:    #I add this
            print("reset: "+self.id+" malfunction")
            self.eng.trafficlight.setProgram(self.id, "off")
    ...
    def prep_phase(self, new_phase):
        ...
        '''
        # if self.disabled:   #QCY: I add this
        #     print("prep_phase "+self.id+" malfunction")
        #     self.eng.trafficlight.setProgram(self.id, "off")
        #     return
    ...
    def _change_phase(self, phase):
        ...
        '''
        # if self.disabled:    #I add this
        #     print("_change_phase " + self.id + " malfunction")
        #     self.eng.trafficlight.setProgram(self.id, "off")
        #     return
        if self.interface_flag:
    ...
    def pseudo_step(self, action):
        ...
        # QCY: if disabled, set phase to all green
        # print(self.disabled,"***")
        if self.disabled:
            # traci.trafficlight.setRedYellowGreenState(self.id, "gggggggggggggggggggggggggggggggggggg")
            # print("pseudo_step " + self.id + " malfunction")
            self.eng.trafficlight.setProgram(self.id,"off")
    ...
    def observe(self, step_length, distance):
        ...
        if self.id in malfunction_intersections:    #QCY: I add this
        # if True:
            for lane in self.in_lanes:
                lane_obs = self.full_observation[lane]
                vs = lane_obs["vehicles"]
                for v in vs:
                    self.comingVehicles.add(v["name"])
            for lane in self.out_lanes:
                lane_obs = self.full_observation[lane]
                vs = lane_obs["vehicles"]
                for v in vs:
                    name = v["name"]
                    if name in self.comingVehicles:
                        self.passedVehicles.add(name)
...
class World(object):
    ...
    def __init__(self, sumo_config, placeholder=0, **kwargs):
        ...
        sumo_dict['gui']="False"     #QCY: I add this


        if sumo_dict['gui'] == "True":
            print("YQC: you will use sumo-gui~")
            sumo_cmd = [sumolib.checkBinary('sumo-gui')]
        else:
            print("YQC: you will use sumo~")
        ...
        self.max_distance = 1000 # TODO: set in registry  #QCY: change from 200 to 1000
        ...
        if malfunction:  # if malfunction, keep some intersections malfunctioning
            for intersection in self.intersections:
                if intersection.id in malfunction_intersections:
                    ...
                    intersection.eng.trafficlight.setProgram(intersection.id,"off")
            for intersection in self.intersections:
