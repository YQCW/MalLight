...
def build_index_intersection_map_hz4x4(roadnet_file):
    #code by QCY temporarily for colight on hz4x4
    ret = dict()
    ...
    for j in range(1,5):
        for i in range(1,5):
            tmp["intersection_"+str(i)+"_"+str(j)] = idx
            idx += 1
    ret["node_id2idx"] = tmp

    ret["sparse_adj"] = np.array([[ 0,  1],
       [ 0,  4],
       [ 1,  0],
       [ 1,  2],
       [ 1,  5],
...
def build_index_intersection_map(roadnet_file):
    #QCY: modify here
    if roadnet_file == "data/raw_data/hangzhou_4x4_gudang_18041610_1h/hangzhou_4x4_gudang_18041610_1h_2.net.xml":
        return build_index_intersection_map_hz4x4(roadnet_file)
    # if roadnet_file == "raw_data/manhattan_7x7_merged_p0.5/manhattan_7x7_3.net.xml":
    roadnet_file = "data/raw_data/manhattan_7x7_merged_p0.5/cityflow_net.json"
    """
