...
class PRLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        #  get generator for PRLightAgent
        ...
        #  get reward generator for PRLightAgent
        ...
        #  get queue generator for PRLightAgent
        queues = []
        for inter in self.world.intersections:
            node_id = inter.id if 'GS_' not in inter.id else inter.id[3:]
            node_idx = self.graph['node_id2idx'][node_id]
        ...
        #  get delay generator for PRLightAgent
        delays = []
        for inter in self.world.intersections:
            node_id = inter.id if 'GS_' not in inter.id else inter.id[3:]
            node_idx = self.graph['node_id2idx'][node_id]
