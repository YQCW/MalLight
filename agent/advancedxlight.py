...
@Registry.register_model('advancedxlight')
class AdvancedXLightAgent(RLAgent):
    '''
    AdvancedXLightAgent coordinates traffic signals by learning Max Pressure based on running and waiting vehicles
    '''
    def __init__(self, world, rank):
        ...
        print("QCY: inter_id: ",inter_id)
        inter_obj = self.world.id2intersection[inter_id]
        self.ob_generator = LaneVehicleGenerator(world, inter_obj, ["lane_count"], average=None)
        self.ob_generator2 = LaneVehicleGenerator(world, inter_obj, ["lane_waiting_count"], average=None)
        ...
        # QCY: put the model on GPU
        # self.ngpu = 0
        # self.device = torch.device("cuda:0" if (torch.cuda.is_available() and self.ngpu > -1) else "cpu")
        # print("QCY: device: " + str(self.device))
    ...
    def get_ob(self):
        ...
        '''
        x_obs = []
        x_obs.append(self.ob_generator.generate()+self.ob_generator2.generate())
