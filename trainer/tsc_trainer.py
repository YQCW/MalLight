...
# malfunction = True
malfunction_intersections = ["intersection_4_2",
                            "intersection_6_2",
                            "intersection_4_5",
                            "intersection_2_6"]
# malfunction_intersections = ["intersection_2_3"]
...
class TSCTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # malfunction_intersections = ["intersection_3_6",
        #                              "intersection_5_6",
        #                              "intersection_3_3",
        #                              "intersection_5_3"]
        # if malfunction:     #if malfunction, keep some intersections malfunctioning
        #     for intersection in self.env.world.intersections:
        #         if intersection.id in malfunction_intersections:
        #             intersection.disabled = True
        #     for intersection in self.env.world.intersections:
        #         if intersection.disabled:
        #             print("intersection "+intersection.id+" malfunction~~~")
    ...
    def create_agents(self):
        ...
        #QCY: I add colight directly to the Registry
        from agent.prlight import CoLightAgent
        Registry.mapping['model_mapping']["colight"] = CoLightAgent
        from agent.mallight import MalLightAgent
        Registry.mapping["model_mapping"]["mallight"] = MalLightAgent
        from agent.advancedxlight import AdvancedXLightAgent
        Registry.mapping["model_mapping"]["advancedxlight"] = AdvancedXLightAgent

        agent = Registry.mapping['model_mapping'][Registry.mapping['command_mapping']['setting'].param['agent']](self.world, 0)
        print("QCY: agent:",agent)
        num_agent = int(len(self.world.intersections) / agent.sub_agents)
        print("QCY: num_agent: ",num_agent)
        ...
        print("QCY: model_mapping setting name: ",Registry.mapping['model_mapping']['setting'].param['name'])
        ...
        for e in range(self.episodes):
            ...
            while i < self.steps:
                if i % self.action_interval == 0:
                    #QCY: print the step number
                    print("QCY: training step"+str(i),' ',datetime.now())

                    #QCY: get current phase from environment
                    last_phase = np.stack([ag.get_phase() for ag in self.agents])  # [agent, intersections]

                    if total_decision_num > self.learning_start:
                        actions = []
                        for idx, ag in enumerate(self.agents):     #QCY：用模型选phase
                            actions.append(ag.get_action(last_obs[idx], last_phase[idx], test=False))                            
                        actions = np.stack(actions)  # [agent, intersections]
                    else:
                        actions = np.stack([ag.sample() for ag in self.agents])     #QCY:随机选phase

                    actions_prob = []   #QCY: 这个干啥用的呀，PressLight返回几个None
                    for idx, ag in enumerate(self.agents):
                        actions_prob.append(ag.get_action_prob(last_obs[idx], last_phase[idx]))

                    # QCY: randomly make an intersection malfunction
                    # if malfunction==True:
                    #     for intersection in self.env.world.intersections:
                    #         if intersection.disabled:
                    #             trans = (random.randint(0, 1000) < 20)
                    #             if trans:
                    #                 intersection.disabled = False
                    #                 print(intersection.id + " repaired")
                    #                 #QCY: 重新load program
                    #                 self.env.eng.trafficlight.setProgram(intersection.id, "0")
                    #         else:
                    #             trans = (random.randint(0, 1000) < 3)
                    #             if trans:
                    #                 intersection.disabled = True
                    #                 print(intersection.id + " malfunction")

                    rewards_list = []   #QCY: 环境采取action并前进一个interval，收集ob与reward信息
                    ...
                    for idx, ag in enumerate(self.agents):         #QCY: 将sample加入到replay memory
                        if not ag.get_intersection().disabled:
                ...
                if total_decision_num > self.learning_start and\
                    ...
                    #QCY: i modify the training logic
                    cur_loss_q = np.stack([ag.train() for ag in self.agents if not ag.get_intersection().disabled])
                    # cur_loss_q = np.stack([ag.train() for ag in self.agents])  # TODO: training    #QCY: why is it TODO?

                    episode_loss.append(cur_loss_q)
                if total_decision_num > self.learning_start and \
                        total_decision_num % self.update_target_rate == self.update_target_rate - 1:
                    [ag.update_target_network() for ag in self.agents if not ag.get_intersection().disabled]
            ...
            int_mean_throughputs = []
            for intersection in self.world.intersections:
                if intersection.id in malfunction_intersections:
                    int_mean_throughputs.append(len(intersection.passedVehicles))
                    print(intersection.id, " throughput:", len(intersection.passedVehicles))
            int_mean_throughputs = int(sum(int_mean_throughputs) / len(int_mean_throughputs))
            ...
            self.logger.info("step:{}/{}, q_loss:{}, rewards:{}, queue:{}, delay:{}, throughput:{}, int_mean_throughput:{}".format(i, self.steps,\
                mean_loss, self.metric.rewards(), self.metric.queue(), self.metric.delay(), int(self.metric.throughput()), int(int_mean_throughputs)))
    ...
    def train_test(self, e):
        ...
        for i in range(self.test_steps):
            if i % self.action_interval == 0:
                # QCY: print the step number
                print("QCY: training testing step" + str(i), ' ', datetime.now())
        ...
        self.logger.info("Test step:{}/{}, travel time :{}, rewards:{}, queue:{}, delay:{}, throughput:{}, int_mean_throughput:{}".format(\
            e, self.episodes, self.metric.real_average_travel_time(), self.metric.rewards(),\
            self.metric.queue(), self.metric.delay(), int(self.metric.throughput()), int_mean_throughputs))
    ...
    def test(self, drop_load=True):
        ...
        for i in range(self.test_steps):
            if i % self.action_interval == 0:
                # QCY: print the step number
                print("QCY: testing step" + str(i), ' ', datetime.now())
        ...
        self.logger.info("Final Travel Time is %.4f, mean rewards: %.4f, queue: %.4f, delay: %.4f, throughput: %d, int_mean_throughput: %d" % (self.metric.real_average_travel_time(), \
            self.metric.rewards(), self.metric.queue(), self.metric.delay(), self.metric.throughput(), int_mean_throughputs))
        return self.metric
